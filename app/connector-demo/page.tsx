...
const ConnectorDemoPage: NextPage = () => {
  ...
  const [pathType, setPathType] = useState<'curvePath' | 'straightPath'>(
    'curvePath',
  );
  const [arrowTip, setArrowTip] = useState<'none' | 'start' | 'end' | 'both'>(
    'both',
  );
  ...
  useEffect(() => {
    ...
    const connector = canvas
      .getObjects()
      .find((obj) => (obj as any).id === selectedConnector) as XConnector;
  ...
  const onLoad = useCallback((canvas: XCanvas) => {
    ...
    const connector3 = new XConnector(
      {
        x: diamond1.left! + diamond1.width! / 2,
        y: diamond1.top! + diamond1.height!,
      },
      { x: rect2.left! + rect2.width! / 2, y: rect2.top! },
      {
        x: diamond1.left! + diamond1.width! / 2,
        y: diamond1.top! + diamond1.height! + 30,
      },
    ...
    const connector4 = new XConnector(
      ...
      {
        x: circle1.left! - circle1.radius! - 20,
        y: circle1.top! + circle1.radius!,
      },
      {
    ...
    // Add instructions
    const instructions = new XTextbox(
      'Select a connector to modify its properties or create new connectors using the panel on the right.\nYou can also drag connector endpoints to attach them to shapes.',
      {
        top: centerY + 300,
        left: centerX,
        originX: 'center',
  ...
  const createNewConnector = () => {
    ...
    // Connect to the selected object if in automatic mode
      ...
      const object = canvas
        .getObjects()
        .find((obj) => (obj as any).id === objectToConnect);
      if (!object) return;
      ...
      const otherObjects = canvas
        ...
        .filter(
          (obj) =>
            (obj as any).id !== objectToConnect &&
            (obj.type === 'XRectNotes' ||
              obj.type === 'XCircleNotes' ||
              obj.type === 'XShapeNotes'),
        );
      ...
      const targetCenterX =
        targetObject.left! + (targetObject.width ? targetObject.width / 2 : 0);
      const targetCenterY =
        targetObject.top! + (targetObject.height ? targetObject.height / 2 : 0);
  ...
  const deleteSelectedConnector = () => {
    ...
    const connector = canvas
      .getObjects()
      .find((obj) => (obj as any).id === selectedConnector);
      ...
      >
        ...
        <Stack spacing={2}>
          ...
          <Select
            value={pathType}
            onChange={(_, value) =>
              setPathType(value as 'curvePath' | 'straightPath')
            }
          >
          ...
          <Select
            value={arrowTip}
            onChange={(_, value) =>
              setArrowTip(value as 'none' | 'start' | 'end' | 'both')
            }
          >
